---
title: "Body-sites microbiome in animal model: drug repurposing TB_Review"
subtitle: "Figure 2A,C,D, Figure 3 and Figure 4"
...
##Figure 2
###Fig 2A: Beta diversity body-sites
...
s_sites %>%
  ...
  geom_point(
    aes(fill=study_group, shape=SampleType), size=5) +
  scale_shape_manual(values = c(23, 24, 21)) +
  scale_fill_manual(values = cols_group, guide = guide_legend(override.aes = list(shape = 22))) +
  geom_text_repel(
 ...
 #theme(legend.position="bottom") +
  ...
  xlim(-0.7, 0.7) +
  ylim(-0.7, 0.7) 
#ggsave("figures_revised/sites_braycurtis.pdf", width=7, height=5, useDingbats=F)
```

```{r test, include=FALSE}
source("../Functions/Function.Pairwise.Adonis.dm.R")
...
###Fig S2C: Beta-diversity group color
```{r Plot PCoA Body.sites, fig.width=7, fig.height=5}
s_sites %>%
  left_join(s_coords, by="SampleID") %>%
  ggplot(aes(x=Axis.1, y=Axis.2)) +
...
##Fig S1: Metadata PLOTS
###Fig S1B: Caries levels 
```{r}
...
## Figure 4 Nestedness and turnover
```{r}
s_nest <- s_sites %>%
  droplevels()
```
...
jacc_nest_cage_lm %>%
  ...
  ggplot(aes(x=term, color=Component)) +
  ...
  theme(
    ...
    strip.text.y = element_text(angle=0, hjust = 0))
ggsave(
  "Figures/sites_cage_nestedness_lm.pdf",
  width=8, height=2, useDingbats=F)
ggsave(
  "Figures/sites_cage_nestedness_lm.png",
  width=8, height=2, dpi=200)
```
...
ggsave("Figures/sites_nestedness_frac.pdf", width=8, height=4, useDingbats=F)
```

###Fig 4A: Shared species
...
ggsave(
  "Figures/sites_taxon_props_OralDentalPaired.pdf",
  width=8.5, height=6.5, useDingbats=F)
ggsave(
  "Figures/sites_taxon_props_OralDentalPaired.png",
...
ggsave(
 "Figures/sites_figure4b_DentalOral.pdf",
  width=4.5, height=5.5, useDingbats=F)
ggsave(
  "Figures/sites_figure4b_DentalOral.png",
...
paired_pres %>%
  ...
  labs(y="Number of ASVs", x="Mouse ID", fill="") +
  theme_boxplot() +
  theme(
...
#ggsave(
#  "Figures/sites_taxon_pres_tb.pdf",
#  width=6, height=6, useDingbats=F)
#ggsave(
#  "Figures/sites_taxon_pres_tb.png",
...
subj_pairs_feces <- s_sites %>%
  filter(!(SampleType %in% "Dental Plaque")) %>%
  pivot_wider(
    id_cols = Cage.N,
    names_from = SampleType,
    values_from = SampleID) %>%
...
ggsave(
  "Figures/sites_taxon_props_OralFecesPaired.pdf",
  width=8.5, height=6.5, useDingbats=F)
ggsave(
  "Figures/sites_taxon_props_OralFecesPaired.png",
  width=8.5, height=6.5, dpi=200)
```
...
paired_props_feces %>%
  ...
  #theme(strip.background = element_blank())
ggsave(
  "Figures/sites_figure4b_OralFeces.pdf",
  width=4.5, height=5.5, useDingbats=F)
ggsave(
  "Figures/sites_figure4b_OralFeces.png",
  width=4.5, height=5.5, dpi=200)
```
...
paired_pres_feces %>%
  ...
  theme(
    ...
    strip.background = element_blank())
ggsave(
  "Figures/sites_taxon_pres_tb_feces.pdf",
  width=6, height=6, useDingbats=F)
ggsave(
  "Figures/sites_taxon_pres_tb_feces.png",
  width=6, height=6, dpi=200)
```


```{r}
shared_colors <- c("#333333", "#32a3e6", "#8AC7EB", "#f0d17d")
...
paired_pres_feces %>%
  ...
  facet_wrap(AssignmentLabel ~ study_group, scales = "free") +
...
ggsave(
  "inst/figures/sites_figure4b.pdf",
  width=4.4, height=2, useDingbats=F)
ggsave(
  "inst/figures/sites_figure4b.png",
  width=4.4, height=2, dpi=200)
```
...
ggsave(
  "Figures/sites_taxon_props_FecesPlaquePaired.pdf",
  width=8.5, height=6.5, useDingbats=F)
ggsave(
  "Figures/sites_taxon_props_FecesPlaquePaired.png",
  width=8.5, height=6.5, dpi=200)
```
...
paired_pres_feces_plaque %>%
  ...
  theme(
    ...
    strip.background = element_blank())
ggsave(
  "Figures/sites_taxon_pres_tb_feces_plaque.pdf",
  width=6, height=6, useDingbats=F)
ggsave(
  "Figures/sites_taxon_pres_tb_feces_plaque.png",
  width=6, height=6, dpi=200)
```
...
paired_pres_feces_plaque_glm %>%
  ...
  theme(
    ...
    strip.text.y = element_text(angle=0, hjust=0))
ggsave(
  "Figure/sites_taxon_pres_tb_feces_plaque_lm.pdf",
  width=5, height=2, useDingbats=F)
ggsave(
...
```{r}
shared_colors <- c("#333333", "#6ad54a", "#4743c7")
oral_dental_asvs_shared %>%
  select(-Total) %>%
  pivot_longer(
    -c(AssignmentLabel, Comparison), 
    names_to = "AsvType", values_to = "n") %>%
  group_by(AssignmentLabel, Comparison) %>%
  mutate(p = n / sum(n)) %>%
  ungroup() %>%
  mutate(p_shared = ifelse(AsvType %in% "Shared", p, 0)) %>%
  mutate(AssignmentLabel = fct_reorder(
    AssignmentLabel, p_shared, .fun = max)) %>%
  mutate(
    AsvType = str_replace(AsvType, "Other site", "Dental Plaque")) %>%
  mutate(
    AsvType = fct_relevel(AsvType, "Shared", "Oral Swab only")) %>%
  ggplot() +
  geom_col(
  ...
  labs(
    y="Percentage of 16S amplicon sequence\nvariants shared between body sites",
    x="", fill="") +
  theme_boxplot() +
  theme(
...
ggsave(
  "Figures/sites_plaque_taxon_percentage_shared.pdf",
  width=5, height=3, useDingbats=F)
ggsave(
  "Figures/sites_plaque_taxon_percentage_shared.png",
...
```{r}
shared_colors_feces <- c("#333333", "#6ad54a", "#ae1c00")
oral_feces_asvs_shared %>%
  select(-Total) %>%
  pivot_longer(
    -c(AssignmentLabel, Comparison), 
    names_to = "AsvType", values_to = "n") %>%
  ...
  mutate(AssignmentLabel = fct_reorder(
    AssignmentLabel, p_shared, .fun = max)) %>%
  mutate(
    AsvType = str_replace(AsvType, "Other site", "Feces")) %>%
  mutate(
    AsvType = fct_relevel(AsvType, "Shared", "Oral Swab only")) %>%
  ggplot() +
  geom_col(
  ...
  coord_flip() +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values=shared_colors_feces) +
  labs(
    y="Percentage of 16S amplicon sequence\nvariants shared between body sites",
    x="", fill="") +
  theme_boxplot() +
...
ggsave(
  "Figures/4A_Oral_Feces.pdf",
  width=5, height=3, useDingbats=F)
ggsave(
  "Figures/4A_Oral_Feces.png",
  width=5, height=3, dpi=200)
```
...
ggsave(
  "Figures/sites_figure4a.pdf",
  width=10.5, height=5, useDingbats=F)
ggsave(
  "Figures/sites_figure4a.png",
